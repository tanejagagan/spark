...
private[sql] class SharedState(val sparkContext: SparkContext) extends Logging {
  ...
  protected val externalCatalogCreateFn =
    ...
    def apply(domain: String): ExternalCatalogWithListener = {
      val clonedConfig = new Configuration(sparkContext.hadoopConfiguration);
      val configFile = Utils.getContextOrSparkClassLoader.getResource(s"${domain}-hive-site.xml")
      if (configFile != null) {
        logInfo(s"overloading hive config file: $configFile")
        clonedConfig.addResource(configFile)
      }
      if(configFile == null && domain != StaticSQLConf.DEFAULT_SQL_DOMAIN.defaultValueString) {
        throw new RuntimeException(s"configuration for the domain '${domain}' is not found")
      }
      ...
      val externalCatalog = SharedState.reflect[ExternalCatalog,
        ...
        sparkContext.conf,
        clonedConfig, domain)
      // load domain specific configuration file
