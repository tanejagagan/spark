...
private[spark] object HiveUtils extends Logging {
  ...
  private def formatConnectionWarehouseString(conf : SparkConf,
    ...
    val metastroreProps = if ( connectionUrl != null ) {
      Map(ConfVars.METASTORECONNECTURLKEY.varname -> connectionUrl.replaceAll(
        "\\$\\{domain\\}", domain))
    } else {
      Map()
    }
    warehouseProps ++ metastroreProps
  }
  /**
  ...
  def newTemporaryConfiguration(useInMemoryDerby: Boolean): Map[String, String] = {
    ...
    val localMetastore = new File(tempDir, "metastore-${domain}/")
